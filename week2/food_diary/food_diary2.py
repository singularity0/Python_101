            ...
            try:
                ...
                if command == "meal":
                    ...
                    if not (quantity.endswith("kg") or quantity.endswith("g")):
                        raise Exception
                    quantity = self._calories_calculator.calculate_quantity(
                        quantity)
                    if not self._calories_calculator.has_meal(parameter):
                        print(self._create_calories_for_meal(parameter))
                        calories_for_meal = input(
                            "How much calories per 100g?> ")
                        self._calories_calculator.add_meal_calories(
                            parameter, calories_for_meal)
                    calories = self._calories_calculator.calculate_calories(
                        parameter, quantity)
                ...
                print(
                    "You entered invalid command. Please check the help menu for more information.")
