    ...
    def transfer_to(self, account, amount):
        if self._balance >= amount:
            if self.currency == account.currency:
                self._balance -= amount
                self._history.append("The amount of {} transferred to the {} account".format(amount, account.name))
                account._history.append("Recevied {} from source {}".format(amount, self.name))
